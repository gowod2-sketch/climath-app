...
확인할 것:
- 푸시되지 않은 커밋이 남아 있는가
- 대상 사이트가 **git에 연결돼 있는가** — 없으면 push로는 배포되지 않는다
- 마지막 배포 커밋이 현재 HEAD와 일치하는가

`netlify` CLI가 없으면 스크립트가 `exit 2`와 함께 수동 확인 절차를 알려준다. **미확인을 "확인됨"으로 보고하지 마라.**

그 다음:
1. `git status`로 `index.html` 변경 여부 확인 (build.py가 데이터 블록을 갈아끼운다)
2. `_workspace/` 보존
3. 생성/수정 파일 목록, 미해결 에스컬레이션, **배포 도달 여부**를 요약 보고
