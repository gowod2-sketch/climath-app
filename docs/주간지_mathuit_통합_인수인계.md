...
| 상태 | 설계만 확정, 코드 미착수 | 코드 4개 작성 완료, 미검증 다수 |
...
| 즐겨찾기(책갈피) | **신규 기능으로 확정.** 문항 페이지마다 책갈피 버튼을 두고, 켜면 "복습 모드"로 전환해 즐겨찾기(또는 오답) 표시된 문항만 걸러서 보여줌 — 아래 "즐겨찾기/복습 모드" 절 참고 |

### 즐겨찾기(책갈피) → 복습 모드 (2026-09-06 추가 결정)

학생·교사 둘 다 문항 페이지에서 책갈피 버튼으로 특정 문항을 표시해두고,
나중에 "복습 모드"로 전환하면 표시된 문항만 걸러서 이어보기가 가능해야
한다.
...
- **필터 기준**: 즐겨찾기와 "오답 표시"(트랙 2 QR 오답체크로 쌓인 데이터)가
  같은 개념인지 별개인지 정할 것. 지금 둘은 서로 다른 시스템(로컬 즐겨찾기
  vs Supabase `weekly_wrong_answers`)이라 자동으로 안 이어진다 — 이어붙이면
...
- **UI**: `buildTOC()`가 이미 `pages[]`를 필터링해 보여주는 구조이므로,
  복습 모드는 같은 컴포넌트에 "즐겨찾기만" 필터를 얹는 것으로 구현 가능해
  보임 — 새 UI를 새로 만들기보다 TOC 필터링 로직 재사용을 먼저 검토할 것.
...
- **퀴즈(문항) 자리 — 있는데 완성이 안 됐다. 이게 핵심 발견.**
  ...
  - CSS·상호작용도 있음 — `.quizopt`(보기 클릭), `selectQuizOption()`(클릭 시
    체크 표시+사운드)
  - **그런데 실제로 그리는 함수(`index.html` 약 251번째 줄)에
    `var quiz='';`로 선언만 있고 채우는 코드가 없다.** 지금 퀴즈 페이지를
    열면 제목만 뜨고 문제·보기가 안 보임 (244번째 줄 더미 문항도 마찬가지)
  - 커밋 `2f99d5c`("feat: support direct concept and problem page
    registration", 2026-08-13) 이후 손 안 댐. **처음부터 새로 만들지 말고
    이 미완성 기능을 완성하는 쪽으로 접근할 것.**
  - 지금 스키마엔 **해설(sol)·출처(src)·모의고사 배지 필드가 없음** — 주간지에
    필요한 만큼 확장해야 함
...
2. ~~교사/학생 화면 차이~~ → **확정됨**: 교사 계정은 해설 노출, 학생 계정은
   ...
   시 `sessionStorage`에 role 플래그를 심고 렌더링 시 분기하는 정도로
   충분해 보이나, `quiz` 렌더링 자체가 아직 없어서(§ mathuit 실측 사실)
   그 안에서 함께 설계할 것
3. 수식 렌더러 통일 방식 (문항 페이지만 MathJax 예외로 둘지, 전체 이관할지)
4. `quiz` 렌더링 완성 — `index.html`의 빈 `var quiz=''` 채우기 + `sol`·`src`·
   `모의` 필드 추가, **더해서 이제 교사/학생 분기 렌더링과 즐겨찾기 버튼까지
   같이 설계해야 함**(위 "즐겨찾기(책갈피) → 복습 모드" 절 참고) — 이번
   통합의 핵심 코드 변경이 셋으로 늘어난 셈이니 한 번에 설계하고 나눠 구현할 것
