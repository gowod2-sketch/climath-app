...
- **퀴즈(문항) 자리 — 2026-09-06 정정: 실제로는 이미 완성돼 있었다.**
  ...
  **틀렸다** — 실측 없이 코드를 겉핥기로 읽은 결과였다. 실제로 확인하니:
  ...
  - 남은 것: 교사/학생 해설 노출 분기(§ "확정된 것" 표, 로그인 role별 표시
    ...
    목차 2단 구조 — 전부 미착수
  - **2026-09-06 추가 — `main`에 머지해서 실제로 라이브 배포됨**
    (mathuit-textbook.netlify.app, 커밋 `184ddfc`→`6e3d245`). 실기기에서
    MathJax 렌더링 확인됨(라텍스 정상 렌더 — 샌드박스 우려는 기우였음).
    이어서 사용자 피드백으로 두 가지 추가 반영:
    - **문제 정보 한 줄 헤더**: 번호(`.probno`)+유형명(`.probtype`)을
      한 줄에, 출처(`.probsrc`)는 그 줄 오른쪽 끝 — 인쇄본 `\probcore`의
      "번호+hfill+연도" 배치와 같은 감각. 문제 본문(`.qbig`) 폰트도
      15px→20px로 키움.
    - **여러 교재 탭 신설**: 처음엔 quiz 페이지를 그냥 기존 `PAGES`
      배열 끝(order:900)에 붙여서 "수열의 극한" 스와이프 덱 안에 섞여
      들어갔다 — 사용자가 "왜 저기다 머지 시켰지, 새 교재 탭 안
      만들고"라고 정확히 지적함. 목차(TOC) 패널 안에 "교과서"/"CLIMATH
      주간지" 탭 두 개를 추가해서: 탭을 누르면 그 교재 첫 페이지로
      바로 점프하고, 목차 목록도 그 교재 페이지만 필터링해 보여줌.
      `unit` 값으로 book을 가른다(`bookKey()`: `CLIMATH 주간지`→
      `weekly`, 그 외 전부→`main`). 목차를 열 때 현재 페이지의 교재로
      자동 동기화됨.
      **한계**: 좌우 스와이프(드래그 넘기기) 자체는 여전히 하나의 연속
      배열이다 — `idx`/`prog`/`dots`/`animateTo()`가 전역 `PAGES` 배열
      하나를 가정하고 있어서, 탭으로 점프는 되지만 스와이프로 교재
      경계를 완전히 막지는 못한다(교과서 마지막 페이지에서 계속 스와이프
      하면 주간지로 넘어갈 수 있음). 진짜 분리(교재별 로컬 인덱스 공간)
      는 `apply()`/`settle()`/`animateTo()`/dots 초기화/`saveLast()`
      전부를 book-스코프로 다시 짜야 하는 더 큰 리팩터라 이번엔 안 함 —
      교재가 3개 이상으로 늘거나 사용자가 스와이프 누출을 실제 문제로
      느끼면 다음 세션이 처리할 것.
